...
namespace GameLift
{
 ...
 public:
     ...
     void SetErrorName(const char* errorName) { strcpy_s(m_errorName, errorName); }

     const char* GetErrorMessage() const { return m_errorMessage; }
     void SetErrorMessage(const char* errorMessage) { strcpy_s(m_errorMessage, errorMessage); }
